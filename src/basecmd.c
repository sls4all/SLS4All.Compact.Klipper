...
static void
move_finalize(uint32_t moves)
{
    if (is_finalized())
        shutdown("Already finalized");
    move_request_size(sizeof(*move_free_list));
    move_list = alloc_chunks(move_item_size, moves, &move_count);
    move_reset();
}
...
{
    sendf("config is_config=%c crc=%u move_count=%hu is_shutdown=%c static_string_id=%hu"
          , is_finalized(), config_crc, move_count, sched_is_shutdown(), sched_get_shutdown_reason());
}
...
{
    move_finalize(1024 /* klipper default */);
    config_crc = args[0];
}
DECL_COMMAND(command_finalize_config, "finalize_config crc=%u");

void
command_finalize_config_with_moves(uint32_t *args)
{
    move_finalize(args[1]);
    config_crc = args[0];
}
DECL_COMMAND(command_finalize_config_with_moves, "finalize_config_with_moves crc=%u move_count=%u");
...
void
command_poweroff(uint32_t *args)
{
    system_poweroff();
}
DECL_COMMAND_FLAGS(command_poweroff, HF_IN_SHUTDOWN, "system_poweroff");
