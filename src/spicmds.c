...
command_spi_set_bus(uint32_t *args)
{
    ...
    uint8_t mode = args[2];
    if (mode > 3 || spi->flags & SF_SOFTWARE)
...
static void
spidev_transfer_with_keep_cs(struct spidev_s *spi, uint8_t receive_data, uint8_t keep_cs
                , uint8_t data_len, uint8_t *data)
{
    ...
    if (spi->flags & SF_HAVE_PIN && !keep_cs)
        gpio_out_write(spi->pin, 1);
    irq_enable();
}
...
spidev_transfer(struct spidev_s *spi, uint8_t receive_data
                , uint8_t data_len, uint8_t *data)
{
    spidev_transfer_with_keep_cs(spi, receive_data, 0, data_len, data);
}

void
spidev_transfer_continuous(struct spidev_s *spi, uint8_t receive_data
                , uint8_t data_len, uint8_t *data)
{
    spidev_transfer_with_keep_cs(spi, receive_data, 1, data_len, data);
}
...
void
command_spi_transfer_continuous(uint32_t *args)
{
    uint8_t oid = args[0];
    struct spidev_s *spi = spidev_oid_lookup(oid);
    uint8_t data_len = args[1];
    uint8_t *data = (void*)(size_t)args[2];
    spidev_transfer_continuous(spi, 1, data_len, data);
    sendf("spi_transfer_response oid=%c response=%*s", oid, data_len, data);
}
DECL_COMMAND(command_spi_transfer_continuous, "spi_transfer_continuous oid=%c data=%*s");
...
void
command_spi_send_continuous(uint32_t *args)
{
    struct spidev_s *spi = spidev_oid_lookup(args[0]);
    uint8_t data_len = args[1];
    uint8_t *data = (void*)(size_t)args[2];
    spidev_transfer_continuous(spi, 0, data_len, data);
}
DECL_COMMAND(command_spi_send_continuous, "spi_send_continuous oid=%c data=%*s");
