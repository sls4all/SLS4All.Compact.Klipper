...
DECL_COMMAND_FLAGS(command_update_digital_out, HF_IN_SHUTDOWN, "update_digital_out_in_shutdown oid=%c value=%c");
...
void
command_update_soft_pwm_out(uint32_t *args)
{
    int ms10 = timer_from_us(10000);
    uint32_t newArgs[4] = { args[0], timer_read_time() + ms10, args[1], args[2] };
    command_schedule_soft_pwm_out(newArgs);
}
DECL_COMMAND(command_update_soft_pwm_out, "update_soft_pwm_out oid=%c on_ticks=%u off_ticks=%u");
