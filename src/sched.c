...
sched_add_timer(struct timer *add)
{
    ...
    if (unlikely(timer_is_before(waketime, timer_list->waketime))) {
        ...
        // NOTE: disabled following check, so we can quickly switch laser and other outputs with PIN_MIN_TIME=0
...
unsigned int
sched_timer_dispatch(timer_func *next_func)
{
...
// Return shutdown reason if the machine is in an emergency stop state
uint8_t
sched_get_shutdown_reason(void)
{
    if (!!shutdown_status)
