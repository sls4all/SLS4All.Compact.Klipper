...
void
command_update_pwm_out(uint32_t *args)
{
    struct pwm_out_s *p = oid_lookup(args[0], command_config_pwm_out);
    gpio_pwm_write(p->pin, args[1]);
}
DECL_COMMAND(command_update_pwm_out, "update_pwm_out oid=%c value=%hu");

void
gpio_oid_pwm_set_noirq(uint8_t oid, uint32_t val)
{
    struct pwm_out_s *p = oid_lookup(oid, command_config_pwm_out);
