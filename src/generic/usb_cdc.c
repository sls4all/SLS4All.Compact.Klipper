...
#include "basecmd.h" // system_poweroff
...
usb_bulk_out_task(void)
{
    ...
    if (ret == 16 && receive_buf[receive_pos+15] == '\n'
        && memcmp(&receive_buf[receive_pos], "SYSTEM_POWEROFF\n", 16) == 0)
    {
        system_poweroff();
        // NOTE: cannot call shutdown() since that may overwrite poweroff pins with shutdown values (if configured)
    }
