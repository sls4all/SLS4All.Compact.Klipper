...
static uint8_t
i2c_read_byte(LPC_I2C_TypeDef *i2c, uint8_t ack, uint32_t timeout)
{
    if (ack) 
        i2c->I2CONSET = IF_ACK; // assert AA -ACK more bytes to come
    else 
        i2c->I2CONCLR = IF_ACK;	
    i2c->I2CONCLR = IF_IRQ;
    i2c_wait(i2c, IF_IRQ, timeout);
         ...
         , uint8_t read_len, uint8_t *read)
{
    ...
    i2c_start(i2c, timeout);
    i2c_send_byte(i2c, config.addr | 1/*read*/, timeout);
    while (read_len--)
        *(read++) = i2c_read_byte(i2c, read_len, timeout);
    i2c_stop(i2c, timeout);
}
...
{
    shutdown("i2c_write_ok not supported");
    return 0;
}
...
{
    shutdown("i2c_read_ok not supported");
    return 0;
}