...
re_pin = re.compile(r'(?P<prefix>[ _]pin=|sensor_pin=)(?P<name>[^ ]*)')
...
class PrinterPins:
    ...
    def lookup_pin(self, pin_desc, can_invert=False, can_pullup=False,
        ...
        return pin_params
    def setup_pin(self, pin_type, pin_desc, sensor_pin_desc=None, control_periods=1):
        can_invert = pin_type in ['endstop', 'digital_out', 'pwm', 'dimmer']
        can_pullup = pin_type in ['endstop']
        pin_params = self.lookup_pin(pin_desc, can_invert, can_pullup)
        if pin_type == 'dimmer':
            dimmer_pin_params = self.lookup_pin(sensor_pin_desc, False, False, share_type="dimmer_sensor")
            pin_params['sensor'] = dimmer_pin_params['pin']
        pin_params['control_periods'] = control_periods
