...
class MCU_digital_out:
    ...
    def _build_config(self):
        ...
        self._force_cmd = self._mcu.lookup_command(
            "set_digital_out pin=%u value=%c", cq=cmd_queue)
    def set_digital(self, print_time, value):
        self._mcu.powerman.set_value(self, print_time, 1. if value else 0.)

    def set_pwm(self, print_time, value, cycle_time=None):
        self._mcu.powerman.set_value(self, print_time, value, cycle_time=cycle_time)
    
    def set_value_callback(self, print_time, value, cycle_time=None):
        self.set_digital_inner(print_time, value >= 0.5)

    def set_digital_inner(self, print_time, value):
        clock = self._mcu.print_time_to_clock(print_time)
...
class MCU_dimmer:
    def __init__(self, mcu, pin_params):
        ...
        self._pin = pin_params['pin']
        self._invert = pin_params['invert']
        self._sensor = pin_params['sensor']
        self._control_periods = pin_params['control_periods']
    ...
    def _build_config(self):
        ...
        self._last_clock = self._mcu.print_time_to_clock(print_time + 0.200)
        self._dimmer_max = self._mcu.get_constant_float("DIMMER_MAX")
        self._oid = self._mcu.create_oid()
        v = int(max(0., min(1., self._start_value)) * self._dimmer_max + 0.5)
        self._mcu.add_config_cmd(
            "config_dimmer_out oid=%d sensor_pin=%s pin=%s value=%hu invert=%d control_periods=%hu"
            % (self._oid, self._sensor, self._pin, v, self._invert, self._control_periods))
        self._set_cmd = self._mcu.lookup_command(
            "schedule_dimmer_out oid=%c clock=%u value=%hu",
            cq=cmd_queue)

    def set_pwm(self, print_time, value):
        self._mcu.powerman.set_value(self, print_time, value)

    def set_dimmer(self, print_time, value):
        self._mcu.powerman.set_value(self, print_time, value)

    def set_value_callback(self, print_time, value, cycle_time=None):
        self.set_dimmer_inner(print_time, value)

    def set_dimmer_inner(self, print_time, value):
        clock = self._mcu.print_time_to_clock(print_time)
        minclock = self._last_clock
        self._last_clock = clock
        v = int(max(0., min(1., value)) * self._dimmer_max + 0.5)
...
class MCU_pwm:
    ...
    def set_value_callback(self, print_time, value, cycle_time=None):
        self.set_pwm_inner(print_time, value, cycle_time=cycle_time)

    def set_pwm_inner(self, print_time, value, cycle_time=None):
        if self._invert:
            value = 1. - value
        if self._hardware_pwm: # hard PWM (has queues after Inova updates)
            clock = self._mcu.print_time_to_clock(print_time)
            sched = self._mcu.print_time_to_clock(print_time - HW_PWM_AHEAD_TIME)
            minclock = self._last_clock
            if minclock > sched:
                minclock = sched
            self._last_clock = sched
            v = int(max(0., min(1., value)) * self._pwm_max + 0.5)
            self._set_cmd.send([self._oid, clock, v],
                               minclock=minclock, reqclock=sched)
        else: # soft PWM (original solution without queues)
            clock = self._mcu.print_time_to_clock(print_time)
            minclock = self._last_clock
            self._last_clock = clock
...
class MCU:
    ...
    def __init__(self, config, clocksync):
        ...
        # powermanager
        self.powerman = self._printer.lookup_object('powerman')
    ...
    def lookup_command(self, msgformat, cq=None, must_exist = True):
        if not must_exist:
            existing = self._serial.get_msgparser().lookup_command(msgformat, must_exist=False)
            if existing is None:
                return None
        return CommandWrapper(self._serial, msgformat, cq)
    def lookup_query_command(self, msgformat, respformat, oid=None,
                             cq=None, is_async=False, must_exist = True):
        if not must_exist:
            existing = self._serial.get_msgparser().lookup_command(msgformat, must_exist=False)
            if existing is None:
                return None
