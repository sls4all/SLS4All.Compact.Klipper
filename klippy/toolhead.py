...
class MoveQueue:
    ...
    def flush(self, lazy=False):
        ...
        for i in range(flush_count-1, -1, -1):
            ...
            if self.toolhead.no_accel:
                cruise_v2 = move.max_cruise_v2
                move.set_junction(cruise_v2, cruise_v2, cruise_v2)
                if update_flush_count:
                    flush_count = i
                    update_flush_count = False
            elif smoothed_v2 < reachable_smoothed_v2:
...
class ToolHead:
    def __init__(self, config):
        ...
        self.no_accel = config.getboolean('no_accel', False)
        ...
        gcode.register_command('GET_REMAINING_MOVES_TIME', self.cmd_GET_REMAINING_MOVES_TIME)
        ...
        # DAC steppers
        gcode.register_command('SET_DAC_STEPPER', self.cmd_SET_DAC_STEPPER)
        gcode.register_command('DAC_PATTERN_ENABLE', self.cmd_DAC_PATTERN_ENABLE)
        gcode.register_command('DAC_PATTERN_SET', self.cmd_DAC_PATTERN_SET)

    def _handle_ready(self):
        self._dac_pattern_enable_cmd = self.mcu.try_lookup_command("dac_pattern_enable enable=%u")
        self._dac_pattern_set_cmd = self.mcu.try_lookup_command("dac_pattern_set index=%u devx=%u x=%u devy=%u y=%u delay=%u")
        self._dac_stepper_set_cmd = self.mcu.try_lookup_command("dac_stepper_set dev=%u value=%u max=%u step=%u ofs=%u")

    def cmd_DAC_PATTERN_ENABLE(self, gcmd):
        enable = gcmd.get_int('ENABLE')
        self._dac_pattern_enable_cmd.send([enable])

    def cmd_DAC_PATTERN_SET(self, gcmd):
        index = gcmd.get_int('INDEX', minval=0)
        devx = gcmd.get_int('DEVX', minval=0)
        x = int(round(gcmd.get_int('X', minval=0) / self.kin.rails[0].steppers[0].get_step_dist()))
        devy = gcmd.get_int('DEVY', minval=0)
        y = int(round(gcmd.get_int('Y', minval=0) / self.kin.rails[1].steppers[0].get_step_dist()))
        delay = self.mcu.seconds_to_clock(gcmd.get_float('DELAY'))
        self._dac_pattern_set_cmd.send([index, devx, x, devy, y, delay])

    def cmd_SET_DAC_STEPPER(self, gcmd):
        dev = gcmd.get_int('DEV', minval=0)
        value = gcmd.get_int('VALUE', minval=0)
        maxv = gcmd.get_int('MAX', minval=0)
        step = gcmd.get_int('STEP', minval=0)
        ofs = gcmd.get_int('OFS')
        self._dac_stepper_set_cmd.send([dev, value, maxv, step, ofs])
    ...
    def _process_moves(self, moves):
        ...
        for move in moves:
            ...
            for cb in move.timing_callbacks:
                callback_delay = cb(next_move_time) 
                if callback_delay:
                    next_move_time = next_move_time + callback_delay
    ...
    def have_moves_finished(self):
        ...
        if (not self.special_queuing_state
            ...
            if self.can_pause:
                return False
        return True
    def get_remaining_moves_time(self):
        self._flush_lookahead()
        if not self.special_queuing_state:
            return 0
        eventtime = self.reactor.monotonic()
        estimate = self.mcu.estimated_print_time(eventtime)
        if self.print_time >= estimate:
            return self.print_time - estimate
        return 0
    def cmd_GET_REMAINING_MOVES_TIME(self, gcmd):
        msg = str(self.get_remaining_moves_time())
    ...
    def register_lookahead_callback(self, callback):
        ...
        if last_move is None:
            callback_delay = callback(self.get_last_move_time())
            if callback_delay:
                self.dwell(callback_delay)
            return
