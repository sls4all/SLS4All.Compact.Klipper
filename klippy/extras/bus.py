...
def resolve_bus_name(mcu, param, bus):
    ...
    if bus is None:
        ...
        bus = rev_enums[0]
    if bus not in enums:
        raise ppins.error("Unknown %s '%s', got '%s'" % (param, bus, ", ".join(enums)))
...
def MCU_SPI_from_config(config, mode, pin_option="cs_pin",
    ...
    cs_pin_params = ppins.lookup_pin(cs_pin, share_type="spi_cs")
...
class MCU_I2C:
    def __init__(self, mcu, bus, addr, speed):
        ...
        self.i2c_write_ok_cmd = self.i2c_read_ok_cmd = None
    def get_oid(self):
        return self.oid
    def get_mcu(self):
        return self.mcu
    ...
    def build_config(self):
        ...
        self.i2c_modify_bits_cmd = self.mcu.lookup_command(
            ...
            cq=self.cmd_queue)
        self.i2c_write_ok_cmd = self.mcu.lookup_query_command(
            "i2c_write_ok oid=%c data=%*s", 
            "i2c_write_ok_response oid=%c ok=%c response=%*s", oid=self.oid,
            cq=self.cmd_queue, 
            must_exist=False)
        self.i2c_read_ok_cmd = self.mcu.lookup_query_command(
            "i2c_read_ok oid=%c reg=%*s read_len=%u",
            "i2c_read_ok_response oid=%c ok=%c response=%*s", oid=self.oid,
            cq=self.cmd_queue,
            must_exist=False)
    ...
    def i2c_modify_bits(self, reg, clear_bits, set_bits,
        ...
        self.i2c_modify_bits_cmd.send([self.oid, reg, clearset],
                                      minclock=minclock, reqclock=reqclock)
    def i2c_write_ok(self, data, minclock=0):
        if self.i2c_write_cmd is None:
            # Send setup message via mcu initialization
            data_msg = "".join(["%02x" % (x,) for x in data])
            self.mcu.add_config_cmd("i2c_write_ok oid=%d data=%s" % (
                self.oid, data_msg), is_init=True)
            return
        return self.i2c_write_ok_cmd.send([self.oid, data], minclock=minclock)
    def i2c_read_ok(self, write, read_len):
        return self.i2c_read_ok_cmd.send([self.oid, write, read_len])
