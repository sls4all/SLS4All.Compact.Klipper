...
class GCodePair:
  def __init__(self, handler, gcmd):
...
class GCodeDispatch:
    ...
    def is_traditional_gcode(self, cmd):
        ...
        try:
            ...
            return False
    def register_command(self, cmd, func, when_not_ready=False, desc=None, no_extended_params=False):
        if func is None:
        ...
        if not self.is_traditional_gcode(cmd) and not no_extended_params:
            origfunc = func
    ...
    def _process_commands(self, commands, lock_inside=False, need_ack=True):
        gcmds = []

        for line in commands:
            gcmd = None

            # proprietary shortcut commands with simple parsing (no regexes and multiple string manipulations)
            if len(line) >= 2 and line[1] == '*':
                ...
                if line[0] == 'A':
                    ...
                    # A*1*2*3   -> G1 X1 Y2 F3 
                    if len(parts) == 3:
                        gcmd = GCodeCommand(self, "G1", line, { 'X': parts[1], 'Y': parts[2] }, need_ack)
                    elif len(parts) == 4:
                        gcmd = GCodeCommand(self, "G1", line, { 'X': parts[1], 'Y': parts[2], 'F': parts[3] }, need_ack)
            ...
            if not lock_inside:
                self._execute_cmd(handler, gcmd, need_ack)
            else:
                gcmds.append(GCodePair(handler, gcmd))

        if lock_inside:
            for i in range(0, len(gcmds), BATCH_SIZE):
                batch = gcmds[i:i+BATCH_SIZE]
            with self.mutex:
                for item in batch:   
                    self._execute_cmd(item.handler, item.gcmd, need_ack)

    def _execute_cmd(self, handler, gcmd, need_ack):
        try:
            handler(gcmd)
    ...
    def run_script(self, script):
        self._process_commands(script.split('\n'), lock_inside=True, need_ack=False)
    def get_mutex(self):
        return self.mutex
    ...
    def cmd_default(self, gcmd):
        ...
        if not cmd:
            ...
            return
        if cmd.startswith("SET_MLX_MATRIX"):
            # Handle SET_MLX_MATRIX gcode with numeric and special characters
            handler = self.gcode_handlers.get("SET_MLX_MATRIX", None)
